...
foreach scan (oppPE_AP PRF1_AP CSS_task3_AP COP_task1_AP FieldMap AP_SE REV_AP_SE t1_mpr_tra_iso_ND)
  if ( ($scan == 'AP_SE') || ($scan == 'REV_AP_SE') ) then
    ...
    set nTRs = 1
    to3d -prefix {$scan}_SBRef.nii.gz -session {$rawDir} -time:zt {$nSlices} {$nTRs} 0 FROM_IMAGE {$dicomDir}????????-ST00?-PHCP{$subj}_V{$scanNum}_7{$scanType}/MR-SE009-BOLD_PRF1_AP_SBRef/*.dcm
    # hard coding MR-SE009-BOLD_PRF1_AP_SBRef to prevent false start problem
