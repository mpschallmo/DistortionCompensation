...
%   - overwrite_mutual_info = binary, whether or not to overwrite mat files
%   in which mututal information is stored, 1 = yes, 0 = no
...
%   - full_dice_mat = matrix with Dice coefficient data
%   - full_minfo_mat = matrix with mutual info data
...
if ~isfield(options, 'overwrite_mutual_info')
    options.overwrite_mutual_info = 0; % 1 = yes, 0 = no
end
...
full_dice_mat = nan(nSubj,numel(analyses),4);
full_minfo_mat = nan(nSubj,numel(analyses));
...
for iS = 1:nSubj
    ...
    for iA = 1:numel(analyses)
        ...
        full_dice_mat(:,iA,1) = dice.(analy_names{iA}).mask;
        full_dice_mat(:,iA,2) = dice.(analy_names{iA}).GMWM;
        
        % then load unifized 3T anat and 7T epi, to calculate mutual information
        mInfo_file = fullfile(options.topDir, 'mInfo.mat');
        
        if options.overwrite_mutual_info || ~exist(mInfo_file,'file')
            ...
            full_minfo_mat(iS,iA) = mutInfo(...
                anat_3T_data(:), epi_7T_data(:));
            
            if iS == nSubj % save data at end
                save(mInfo_file, 'full_minfo_mat');
            end
        else
            if iS == nSubj % just load at the end
                get_mInfo = load(mInfo_file);
                full_minfo_mat = get_mInfo.full_minfo_mat;
            end
        end
...
for use_idx = cond_idx
    if use_idx < 3
        all_data = full_dice_mat(:,:,use_idx);
    elseif use_idx == 3
        all_data = full_minfo_mat;
    else
    ...
    results.ttest.(quant_names{use_idx}) = cell(...
        (size(full_dice_mat,2)-1), size(full_dice_mat,2) );
    all_p = [];
    all_iC = [];
    save_FDR_p{use_idx} = nan((size(full_dice_mat,2)-1), size(full_dice_mat,2));
    % 1 tailed t-tests for all paired comparisons
    for iC1 = 1:(size(full_dice_mat,2)-1)
       for iC2 = (iC1+1):size(full_dice_mat,2)
...
if options.displayFigs
    ...
    for iM = cond_idx
        ...
        if iM < 3
            plot_me = squeeze(full_dice_mat(:,:,iM));
        else
            plot_me = full_minfo_mat;
        end
        which_analysis = repmat([1:numel(analyses)], ...
            [size(full_dice_mat,1) 1]);
        ...
        for iA = 1:numel(analyses)
            figure(h1)
            if iM < 3
                mask_data = full_dice_mat(:,iA,iM);
            else
                mask_data = full_minfo_mat(:,iA);
            end
    ...
    for iA = 1:size(full_dice_mat,2)
        for iM = cond_idx
            % means + error bars
            
            if strcmp(lower(options.which_error_bars),'sem') % show SEM error bars
                if iM < 3
                    use_data = full_dice_mat(:,:,iM);
                    plot_me = full_dice_mat(:,iA,iM);
                else
                    use_data = full_minfo_mat;
                    plot_me = full_minfo_mat(:,iA);
                end
            else strcmp(lower(options.which_error_bars),'morey') % show morey within-subj error bars
                if iM < 3
                    use_data = full_dice_mat(:,:,iM) - ...
                        repmat(nanmean(full_dice_mat(:,:,iM),2),[1 size(full_dice_mat,2)]) + ...
                        repmat(nanmean(nanmean(full_dice_mat(:,:,iM),2),1), ...
                        [size(full_dice_mat,1) size(full_dice_mat,2)]); % subtract individual subject mean, add group mean
                    plot_me = use_data(:,iA);
                else
                    use_data = full_minfo_mat - repmat(nanmean(full_minfo_mat,2),...
                        [1 size(full_minfo_mat,2)]) + repmat(nanmean(nanmean(...
                        full_minfo_mat,2),1), size(full_minfo_mat));
                    plot_me = use_data(:,iA);
                end
            end
            ...
            if options.connect_not_sig && ... % show lines connecting post hoc
                    iA~=size(full_dice_mat,2)
                for iA2 = (iA+1):size(full_dice_mat,2)
            ...
            if iA == numel(analyses)
                if iM < 3
                    y_min = min(nanmean(full_dice_mat(:,:,iM),1));
                    y_max = max(nanmean(full_dice_mat(:,:,iM),1));
                    ylabel('Overlap (Dice coeff.)','color','k');
                else                    
                    y_min = min(nanmean(full_minfo_mat,1));
                    y_max = max(nanmean(full_minfo_mat,1));
...
results.dice = dice;
results.full_dice_mat = full_dice_mat;
results.full_minfo_mat = full_minfo_mat;
