...
3dcopy {$dir3T}/T2w_hires.nii.gz {$rawDir}/3T_anat.nii.gz
mri_convert {$dir3T}/wmparc.mgz {$rawDir}/wmparc.nii.gz

# need to make sure T2 is aligned to wmparc, make it 1mm iso like our FS T1
# mps 20201012
cat_matvec -ONELINE {$dir3T}/transforms/T2wtoT1w.mat > {$rawDir}/T2toT1.1D
3dAllineate -overwrite             \
  -source {$rawDir}/3T_anat.nii.gz \
  -prefix {$rawDir}/3T_anat.nii.gz \
  -master {$rawDir}/wmparc.nii.gz  \
  -1Dmatrix_apply {$rawDir}/T2toT1.1D

# skull strip T2 to match brainmask.mgz (T1) mps 20201013
3dSkullStrip -overwrite           \
  -orig_vol                       \
  -input {$rawDir}/3T_anat.nii.gz \
  -prefix {$rawDir}/3T_anat.nii.gz